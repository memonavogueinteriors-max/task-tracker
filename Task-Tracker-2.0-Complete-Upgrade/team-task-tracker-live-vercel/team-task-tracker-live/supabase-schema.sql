-- TEAM TASK TRACKER — COMPLETE SUPABASE DATABASE
-- Fresh installation only. Existing installations should run supabase-migration-1-3-5.sql.
...
create table if not exists public.companies (
  ...
  owner_notification_email text not null default '',
...
create table if not exists public.members (
  ...
  role text not null check (role in ('owner','manager','sales','developer')),
...
create index if not exists tasks_member_date_priority_idx on public.tasks(member_id, task_date desc, priority);
